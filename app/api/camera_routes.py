...
def detect_faces():
    """
    Receives an image file, detects faces, and returns:
    - Number of faces
    - Face locations (top, right, bottom, left)
    - Face encodings
    - Box color: green if >=1 face, red if 0 faces
    """
    ...
    try:
        ...
        box_color = "green" if len(face_locations) > 0 else "red"

        return jsonify({
            "face_count": len(face_locations),
            "locations": [list(loc) for loc in face_locations],  # JSON safe
...
def recognize_face():
    """
    Receives a face encoding, compares it to the user's saved faces,
    returns whether a match was found with confidence
    """
    data = request.json
    try:
...
def register_face():
    """
    Registers a new face for the current user
    """
    data = request.json
    try:
